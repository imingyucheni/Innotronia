...
- Cover images (`assets/insights/<slug>/cover-en.svg` / `-zh.svg`) are generated by the build. Data charts are made with `python3 scripts/insights_media.py chart spec.json` (spec format at the top of that file) and pasted into the body as `<figure>`.
- `python3 scripts/check_insights.py [slug …]` validates articles (tags, image paths, sources, links, no email addresses).
